/**
 * POST /api/portal/push/unsubscribe {endpoint}
...
export async function POST(req: NextRequest) {
  ...
  try {
    const r = await fetch(`${BMS_API_URL}/api/portal/push-unsubscribe`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-BMS-Secret": BMS_SECRET },
      body: JSON.stringify({ line_user_id: session.lineUserId, endpoint: body.endpoint }),
      cache: "no-store",
    });
    const data = await r.json();
    return NextResponse.json(data, { status: r.ok ? 200 : r.status });
  } catch (e) {
    console.error("[/api/portal/push/unsubscribe]", e);
