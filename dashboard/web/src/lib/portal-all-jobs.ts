...
export interface AllJobs {
  count: number;
  newToday: number; // จำนวนงานที่สแกน+จับคู่วันนี้ (Asia/Bangkok) — ป้าย "New+N" การ์ดงานทั้งหมด
  jobs: SentJob[];
}

export async function getAllJobs(lineUserId: string, limit = 500): Promise<AllJobs> {
  if (!lineUserId) return { count: 0, newToday: 0, jobs: [] };
  ...
  const data = (await res.json()) as { ok: boolean; count?: number; new_today?: number; jobs?: SentJob[] };
  return { count: data.count ?? 0, newToday: data.new_today ?? 0, jobs: data.jobs ?? [] };
}
