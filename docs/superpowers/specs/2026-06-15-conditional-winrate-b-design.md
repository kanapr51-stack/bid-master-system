...
## 10b. B.1 PATCH (2026-06-15, หลัง verify การ์ดจริงนาทม) — เลือกราคาแถวจาก win เป้าหมาย

**ปัญหาที่เจอจริง:** ราคา 3 แถว = winner p25/p75 → ในสนาม disc ลึก+แคบ ทุกแถวกลายเป็นราคาชนะหมด (win% 86-100% แบน, 2 แถวยุบติดกัน 343 บาท) → ตารางไม่ช่วยตัดสินใจ.

**Fix (B.1):** เลิกใช้ winner p25/p75 เป็นราคาแถว → **คำนวณราคาที่ให้ win% = target (75/50/25) ที่สนามปกติ** จาก inverse-CDF:
- `k_mid = ns[len(ns)//2]` (คอลัมน์กลาง = สนามปกติ ≈ mean)
- ราคาเป้า t: `disc = quantile(bids, (t/100)^(1/k_mid))` → `price = budget × (1 − disc/100)` (ราคา = **ข้อมูลจริง**)
- win% คอลัมน์ k: `round((t/100)^(k/k_mid) × 100)` — คอลัมน์กลาง = target เป๊ะ, ซ้าย/ขวากระจายตามโมเดล F_bid^k
- dedupe ราคาซ้ำ + **gate: <2 แถว distinct → None** (สนามแคบ → fallback การ์ดเดิม)

**Signature เปลี่ยน:** `winrate_grid(auctions, budget, targets=(75,50,25))` — **ตัด param `prices`** (คำนวณเอง). `field_and_winrate`/`predict()` เลิกส่ง prices. ตัด `_cdf` เพิ่ม `_quantile`.

**ตัวอย่างจริง (data ต่อเนื่อง, mean=6):**
```
   ผู้ยื่น →     4ราย    6ราย    7ราย
   1,138,622      83%     75%     71%     ← เป้า 75% (ดุสุด ราคาต่ำสุด)
   1,171,015      63%     50%     45%     ← เป้า 50%
   1,218,701      40%     25%     20%     ← เป้า 25% (กำไร ราคาสูงสุด)
```
คอลัมน์กลาง = 75/50/25 เป๊ะ · ราคาไล่ระดับไม่ยุบ.

**ยังไม่แก้ (followup):** ปัญหา #3 เลขงานไม่ตรงบล็อกคู่แข่ง (winner-stats vs `_field_auctions` คนละ query) — UX, แยกประเด็น.
