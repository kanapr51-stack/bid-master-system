...
## งานที่ N+215: ไล่ debug "บอร์ดกดช้าไปหมดทุกปุ่ม" (2026-08-04)

### สถานะ: ✅ วินิจฉัยจบ + ลดผลกระทบเบื้องต้นแล้ว — 🟡 ต้องอัปเกรด VPS ถึงจะแก้ถาวร

### บริบท
คุณกัญจน์แจ้งว่าทุกปุ่มในบอร์ดช้ามาก (แถบเมนูล่าง/หน้าหลัก/ตั้งค่า/ประวัติ/โปรไฟล์) รวมถึงเข้าเว็บก็ช้า — ใช้ debug-mantra ไล่หา root cause

### สิ่งที่ตรวจ (breadcrumb ledger)
1. วัดเวลาตอบสนองจริงบน VPS: `/health` (ไม่ควรแตะ DB เลย) ใช้เวลาถึง **52.7 วินาที**, `/api/portal/customer` (endpoint ที่ทุกหน้าเรียกผ่าน requireOnboarding) 25.5 วินาที, `/api/portal/discover` 23.5 วินาที — ขณะที่ `/api/portal/jobs`/`/api/portal/all-jobs` เร็วปกติ (0.16-0.86s) → ความช้าไม่ผูกกับ query ใดเจาะจง แต่เป็นปัญหาระดับระบบ (system-wide starvation)
2. `nproc` = **1** (VPS มี CPU core เดียว), `load average` ตอนตรวจเจอ **16.33** (1-min) — เกิน capacity ของเครื่อง 16 เท่า
3. RAM รวม 1.9GB, เหลือว่างแค่ 89Mi ตอนตรวจ
4. ไฟล์ DB หลัก (`bms_customers.db`) โต **1.86GB** แล้ว — ใกล้เคียง RAM ทั้งเครื่อง ทำให้ cache ไม่พอ ต้องอ่านดิสก์บ่อย; ตารางใหญ่สุดคือ `cgd_winners` **1,962,467 แถว** (เกือบ 2 ล้าน)
5. timer เบื้องหลัง `bms-line-sender` ยิงทุก 1 นาที (แม้ไม่มีงานค้างก็ spawn process ใหม่ทุกรอบ) แข่ง CPU core เดียวกับ bms-api ตรงๆ
6. ตรวจ `journalctl` ของ line-sender/enrichment-worker ช่วงที่ช้า — งานแต่ละรอบเสร็จเร็ว (< 1 วิ, "No pending items") ไม่ใช่ตัวการหลักเดี่ยวๆ แต่ยิงถี่พอจะซ้ำเติมตอนมีโหลดพร้อมกัน

### Root cause
**VPS เล็กเกินไปเทียบกับข้อมูล/โหลดตอนนี้แล้ว** — 1 vCPU + RAM 1.9GB ต้องรับ (ก) เว็บ API ที่ลูกค้าใช้จริง (ข) ฐานข้อมูล 1.86GB ที่ไม่พอดี RAM cache (ค) timer เบื้องหลังหลายตัวที่ยิงถี่ (เฉพาะ line-sender ทุก 1 นาที) — ตอนโหลดพร้อมกันหลายจุด (เช่น มีคนเปิดบอร์ดพร้อม timer ยิง) เครื่องแย่ง CPU/RAM จนแม้ query ง่ายๆ ก็ค้างเป็นสิบวินาที ไม่ใช่บั๊กจากงาน N+212-214 ที่เพิ่งทำ (แต่ requireOnboarding ที่เพิ่มเข้าไปทุกหน้าก็เพิ่ม 1 query ต่อโหลดหน้าจริง ซ้ำเติมเล็กน้อย)

### Fix / ผล
- ลดความถี่ `bms-line-sender.timer` จาก 1 นาที → 2 นาที (ให้เท่า enrichment-worker) — ลดจำนวนครั้งที่ spawn process ใหม่แข่ง CPU core เดียว, ผลกระทบ UX: ข้อความ LINE อาจช้าขึ้นสูงสุด ~1 นาที (ยอมรับได้ ไม่ใช่ระบบ real-time chat)
- วัดซ้ำหลังแก้: load average ตก 16.33 → 0.20, `/health` 52.7s → 0.004s, `/api/portal/customer` 25.5s → 0.005s, `/api/portal/discover` 23.5s → 1.1s — เร็วขึ้นชัดเจน แต่ **นี่คือการลดความถี่ที่ทำให้เกิด spike ไม่ใช่การแก้ที่ต้นตอ (DB ใหญ่กว่า RAM)**

- ระหว่างตรวจ เจอเพิ่ม: ดิสก์ VPS เต็ม **86%** (เหลือ 7.1GB จาก 52GB) — สาเหตุคือไฟล์ backup เก่าค้าง ~17GB ที่ไม่ได้อยู่ใน retention policy อัตโนมัติ (ของจริงมี `bms-backup.timer` เก็บแค่ 5 วันล่าสุดถูกต้องอยู่แล้ว แต่มีไฟล์ pre-migration snapshot แบบ one-off จากการแก้ schema ช่วง มิ.ย.-ก.ค. ค้างอยู่คนละโฟลเดอร์ไม่มีใครลบ) — คุณกัญจน์ confirm ให้ลบแล้ว ลบเรียบร้อย: ดิสก์ 86%→52% (เหลือ 24GB)
