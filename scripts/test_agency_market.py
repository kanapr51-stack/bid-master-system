...
def _conn():
    ...
    rows = [  # ต.โพนทอง — งานถนนท้องถิ่น (แข่งดุ) ปนงานกรมทางหลวง (ชิดเพดาน)
        ...
        ("L3", "องค์การบริหารส่วนจังหวัดนครพนม", "ก่อสร้างถนน คสล. โพนทอง", "หจก.ท้องถิ่น3", 26.0),
...
def test_classifier():
    ...
    assert ci.agency_market("องค์การบริหารส่วนจังหวัดบึงกาฬ") == "local"
    ...
    print("✅ agency_market classifier (local อปท. / central / None)")


def test_fetch_market_filter():
    c = _conn(); tk = ["ถนน"]
    loc = ci._fetch(c, "นครพนม", tk, subdistrict="โพนทอง", district="บ้านแพง", market="local")
    assert {r["winner"] for r in loc} == {"หจก.ท้องถิ่น1", "หจก.ท้องถิ่น2", "หจก.ท้องถิ่น3"}, loc
    assert {r["discount_pct"] for r in loc} == {30.0, 28.0, 26.0}, loc      # ไม่มี DOH 0.3/0.5
    cen = ci._fetch(c, "นครพนม", tk, subdistrict="โพนทอง", district="บ้านแพง", market="central")
    assert {r["winner"] for r in cen} == {"หจก.ทางหลวง1", "หจก.ทางหลวง2"}, cen
    pooled = ci._fetch(c, "นครพนม", tk, subdistrict="โพนทอง", district="บ้านแพง")  # market=None → เดิม
    assert len(pooled) == 5, pooled
    print("✅ _fetch market filter (local/central/pool back-compat)")
